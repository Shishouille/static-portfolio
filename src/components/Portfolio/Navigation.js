  ...
  return (
    ...
    {!isBigScreen && (
      <StyledMobile>
        ...
        <Icon icon={bxsGrid} onClick={() => setMenu(!menu)} height="1.5em" /></div>)}
        {menu && (
          <nav>
          <Icon icon={bxsGrid} onClick={() => setMenu(!menu)} height="1.5em" />
          <Scroll to="bio" activeClass="is-active" onClick={() => setMenu(!menu)} spy>
            {locale[0]}
          </Scroll>
          <Scroll to="about" activeClass="is-active" onClick={() => setMenu(!menu)} spy>
            {locale[1]}
          </Scroll>
          <Scroll to="whatIDo" activeClass="is-active" onClick={() => setMenu(!menu)} spy>
            {locale[2]}
          </Scroll>
          <Scroll to="projects" activeClass="is-active" onClick={() => setMenu(!menu)} spy>
            {locale[3]}
          </Scroll>
          <Scroll to="contact" activeClass="is-active"  onClick={() => setMenu(!menu)} spy>
            {locale[4]}
          </Scroll>
          </nav>
        )}
